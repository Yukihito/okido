...
object Main {
  ...
  def main(args: Array[String]): Unit = {
    ...
    仮想敵対策ページを作成する(ニンフィア, ポケモン図鑑.find(_.name == "ドリュウズ").get, じしん :: アイアンヘッド :: いわなだれ :: Nil,
      s"""
         |苦手な相手。必ず後攻になる。もし相手がASぶっぱで耐久に振ってない場合はマジカルフレイムで確2とれるのでそれを念頭において勝ち筋を考える。
         |
         |S負けよりこちらが2回行動する前に少なくとも相手に2回相手に行動される。(裏を返せば壁が残っていれば単に殴り合っているだけで勝てる)
         |
         |相手が火力最大でも確2。つまり1回はかならず行動できる。相手の体力がすてに半分以上削られている状態なら勝てる。
         |""".stripMargin).write()
    仮想敵対策ページを作成する(ニンフィア, ポケモン図鑑.find(_.name == "ナットレイ").get, ジャイロボール :: タネマシンガン2 :: タネマシンガン5 :: はたきおとす :: Nil,
      s"""
         |常に先制できる。起点にできる相手。
         |
         |多くの場合でマジカルフレイム2発で倒せる。
         |
         |相手のわざで安定して火力がでるのはジャイロボールだが、それでも多くの場合3回は耐えるので、はじめのターンはめいそうを積んだほうがよい。
         |
         |1回攻撃を食らって様子を見て、3発耐えそうな感じなら2回めいそうを積むと3ターン目で確1狙える。
         |
         |3発耐えそうな場合は、2回めいそうを積んだタイミングでねむって体力を全回復したあとに殴ると良い。
         |""".stripMargin).write()
    仮想敵対策ページを作成する(ニンフィア, ポケモン図鑑.find(_.name == "アイアント").get, アイアンヘッド :: ストーンエッジ :: ばかぢから :: Nil,
      s"""
         |必ず後攻になる。相手のメインウェポンのアイアンヘッドは乱1。一方でこちらのマジカルフレイムも(相手が耐久に振っていなければ)確1なので。ASぶっぱであることに賭けて殴りに行ってもいい。
         |
         |壁があるか、ダイマ残ってれば勝てる相手。普通に対面したらこっちが行動する前に殴り殺される。
         |""".stripMargin).write()
    仮想敵対策ページを作成する(ニンフィア, ポケモン図鑑.find(p => p.name == "コオリッポ" && p.variant == "アイスフェイス").get, つららばり2 :: つららばり5 :: はらだいこ :: Nil,
      s"""
         |はらだいこを積まれると辛い相手。多くの場合先制される。メインウェポンがおそらくつららばりでスキルリンクがないので運良くこちらが1回行動できればラッキー。こちらが先制できるかつ、つららばりの回数が少なければやっと2回行動できる。
         |
         |はらだいこなさそうなら普通に殴ってれば勝てそう。耐久に振りまくられている場合は乱3。積んでる余裕はない。
         |
         |はらだいこ積んでHP半減状態なら運が良ければ1回行動できれば勝てる。
         |""".stripMargin).write()
    仮想敵対策ページを作成する(ニンフィア, ポケモン図鑑.find(p => p.name == "コオリッポ" && p.variant == "ナイスフェイス").get, つららばり2 :: つららばり5 :: はらだいこ :: Nil,
      s"""
         |どうせアイスのときもS勝てないのでどうせ戦うならナイスの状態のときのほうがいい。多くの場合ハイパーボイス2回。
         |
         |はらだいこを積まれていなければ1回ぐらい積んでもいいかもしれない。1回積んでも乱2が確2になるだけなので注意
         |""".stripMargin).write()
    仮想敵対策ページを作成する(ニンフィア, ポケモン図鑑.find(_.name == "バンギラス").get, アイアンヘッド :: ばかぢから :: ストーンエッジ :: かみくだく :: Nil,
      s"""
         |
         |""".stripMargin).write()
  ...
  def 仮想敵対策ページを作成する(自分のポケモン: ポケモン, 相手の種族: 種族, 相手のわざリスト: List[わざ], comment: String = ""): Page = {
